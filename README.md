...
## Шаблонизатор лінки Vipaff

Шлях переходу(редіректу) на продукт повинна відбуватися за таким шаблоном (редірект на продукт та відкриття попапа реїстрації):
`{{redirect_domain}}/?s={{project_id}}&ref={{ref}}&landing={{landing}}&click_id={{click_id}}&sub={{sub}}&source={{source}}&url=%23popup-reg`

Для активації унікального бонуса після редіректу додаємо `&specialBonusId=1045961` де `1045961` - id бонуса який необхідно активувавти. Для тесту необхідно щоб бонус був увімкнено на проді.
Приклад:
`{{redirect_domain}}/?s={{project_id}}&ref={{ref}}&landing={{landing}}&click_id={{click_id}}&sub={{sub}}&source={{source}}&specialBonusId=1045961&url=%23popup-reg`

## Початок роботи

Для роботи з доною збіркою в новому проекті скопіюйте всі файли репозіторію <br>
`git clone <this repo>`
Потім находячись в корні проекту запустіть команду `npm i`, яка встановить все що знаходиться в package.json залежностях.
Після того ви можете використати любу із запропонованих команд збірки (кінцеві файли попадають в папку **app** коріневої деректорії): <br>

`gulp` - базова команда яка запускає збірку для розробки використовуючи browser-sync.

`gulp build` - команда для продакшн збірки проекту. Всі ассети зжаті та оптимізовані для виливки на хостинг.
...
`gulp backend` - команда для бекенд-збірки проекту. Вона позбавлена не потрібних речей з dev-збірки але не зжата для зручності бекендеру.

`gulp zip` - команда збирає ваш готовий код в zip-фрхів.
